...
2. For each task (respect dependencies and `[P]` disjoint-write rules), in order:
   ...
   c. Route: Claude Sonnet implementer (fallback) or Codex via `core/scripts/dispatch.mjs codex`.
   d. Delegate to `task-impl-reviewer` subagent.
   e. On accept → `post-task-sync`; on reject/revise → retry (cap 3).
3. Emit events through `core/scripts/dispatch.mjs append` — NEVER append to `events.ndjson` yourself.
