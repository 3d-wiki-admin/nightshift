...
3. **Rebuild state**: run `nightshift replay tasks/events.ndjson --write` — this removes the question from `open_questions` in state.json and unblocks any approval-required task whose `task_id` is `<ref>`.
