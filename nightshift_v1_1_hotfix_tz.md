...
- ~~**H10** (session.end флуд)~~ — **DONE** в hotfix-2 commit f20c243
- ~~**H11** (rich status dashboard)~~ — **DONE** в hotfix-2 commit 7cd1474
...
- ~~**H14** (pinger vs approval-wait)~~ — **DONE** в hotfix-2 commit ae7c7e0
- ~~**H15** (codex-cli 0.121 flag drift)~~ — **DONE** в commit 9a85eb1
- ~~**H16** (wave-end handoff + fresh Claude per wave)~~ — **DONE** в
  hotfix-3 (commits a96172a, 2afa587, c3dfd67). 6 gpt-5.4 design rounds
  + 3 implementation units + cross-model subagent review per unit. Live
  primitive verified via spike (`claude -p` supports plugin slash commands
  non-interactively). Expected $150-200/overnight savings.

  ### Проблема
  Claude Code не имеет программного `/clear`. Orchestrator-сессия
  живёт всю ночь, накапливая контекст (20 тасков × ~5k observations
  = ~100k/волна, × 5 волн = 500k+). Ближе к концу ночи — 629k+ token
  контекст, деньги и деградация качества.

  Между волнами в overnight-режиме нет user nuance для потери — всё
  канонично на диске. Это идеальное место для reset контекста.

  ### Решение (три слоя)

  **Слой A — schema + skill:**
  - Add `wave.handoff` to `core/schemas/event.schema.json` action.enum.
  - Update `core/skills/wave-orchestrator/SKILL.md`: после emit
    `wave.accepted`, перед возвратом:
    1. Написать `tasks/waves/<N>/handoff-to-next.md` с полями:
       - `## Wave <N> summary` — что сделано (прозой, не JSON)
       - `## Pending from this wave` — incomplete/blocked/paused
       - `## Next wave pointer` — `tasks/waves/<N+1>/manifest.yaml` если
         существует
       - `## Canonical state to re-read` — список файлов для fresh
         Claude (CLAUDE.md, HANDOFF.md, paused.md, tasks/plan.md, и
         handoff-to-next.md)
       - `## Ephemeral nuances` — решения не попавшие в events
         (в autonomous mode обычно пусто, но в mixed mode могут быть)
    2. Emit `wave.handoff` event с payload.handoff_path + .next_manifest.
    3. Exit session (orchestrator не берёт следующую задачу — ждёт
       fresh resurrect от pinger'а).

  **Слой B — pinger «resurrect-fresh» path:**
  - `core/scripts/health-ping.mjs`: добавить ветку ПЕРЕД current
    `claude --continue`:
    - Если `openQuestions()` пуст И последние события содержат
      `wave.handoff` чей `payload.next_manifest` существует на диске
      И не было `task.dispatched` в этой next wave yet
      → spawn FRESH `claude` (БЕЗ `--continue`) в project dir
      → прокачать stdin: `re-orient prompt + "/nightshift:implement"`.
  - Re-orient prompt (генерируется скриптом):
    «Continuing nightshift overnight run. Previous wave <N> accepted.
    Read: CLAUDE.md, HANDOFF.md, handoff-to-next.md, paused.md,
    tasks/waves/<N+1>/manifest.yaml. Then run
    /nightshift:implement on wave <N+1>. Do NOT re-litigate previous
    decisions — trust events.ndjson.»

  **Слой C — event for audit:**
  - `wave.handoff` event в events.ndjson формирует аудит: видно когда
    wave завершена, orchestrator session закрыта, и когда pinger
    resurrect'нул fresh session (`pinger.resurrect` — ещё один новый
    event type? Или просто новый `session.start` от fresh claude
    покроет).

  ### Acceptance

  - New regression test `core/scripts/test/hotfix3-wave-handoff.test.mjs`:
    - F-A: orchestrator emits wave.accepted → check handoff.md +
      wave.handoff event → pinger called → spawns `claude` (not
      `claude --continue`); fake claude records its argv.
    - F-B: wave.accepted without next manifest (last wave) → pinger
      does NOT resurrect — either idles or pings normally.
    - F-C: handoff.md has all required sections (Wave summary,
      Pending, Next pointer, Canonical state, Ephemeral).
    - F-D: emit wave.handoff with missing next_manifest path →
      pinger rejects, logs warning.

  - Live verification (next overnight): token count за всю ночь
    сокращается на ≥50% по сравнению с baseline (текущий single-
    session 629k → expected ≤ 300k total across all waves).

  ### Риски / не фиксит

  - Если events.ndjson не полностью ловит решение (например
    orchestrator принял decision в голове и не emit'нул
    `decision.recorded`), fresh Claude в следующей волне может
    re-litigate. **Это не баг — это фича** нашей дисциплины: всё
    load-bearing через events. Если что-то re-decided — значит events
    недостаточно detailed, и это надо чинить в skill prompt'ах.

  - Handoff-файл ePHEMERAL nuance секция обычно пуста в autonomous
    mode (пользователь не пишет в чате ночью). В mixed mode (днём)
    может быть важна — но там пользователь сам делает `/clear` + паст.

  - Pinger resurrect требует чтобы `claude` был в `$PATH` + стабильно
    работал auth. Это уже требование v1.1.
